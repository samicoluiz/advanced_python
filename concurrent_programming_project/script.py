...
async def main_async(list1, list2, list3):  # Main wrapper for asynchronous example
  s = time.perf_counter()
  # your code goes here
  tasks = [cal_average_async(task) for task in (list1, list2, list3)]
...
def main_threading(list1, list2, list3):  # Main wrapper for threading example
  ...
  threads = []
  for i in lists:
    x = threading.Thread(target=cal_average, args=(i,))
    x.start()
    threads.append(x)
  for t in threads:
    t.join()
...
def main_multiprocessing(list1, list2, list3):  # Main wrapper for multiprocessing example
  ...
  # your code goes here
  lists = (list1, list2, list3)
  processes = [Process(target=cal_average, args=(i,)) for i in lists]
  for i in processes:
    i.start()
